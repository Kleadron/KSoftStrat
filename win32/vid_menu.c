...
// Knightmare- fields for custom mode input
static menuseparator_s	s_customwidth_title[2];
static menuseparator_s	s_customheight_title[2];
static menufield_s		s_customwidth_field[2];
static menufield_s		s_customheight_field[2];
...
static void RefreshCallback( void *unused )
{
	...
	// Knightmare- make sure custom fields are hidden/shown based on video mode
	s_customwidth_title[s_current_menu_index].generic.flags	= (s_mode_list[s_current_menu_index].curvalue != 0) ? QMF_HIDDEN : 0;
	s_customwidth_field[s_current_menu_index].generic.flags	= (s_mode_list[s_current_menu_index].curvalue != 0) ? (QMF_NUMBERSONLY|QMF_HIDDEN) : QMF_NUMBERSONLY;
	s_customheight_title[s_current_menu_index].generic.flags	= (s_mode_list[s_current_menu_index].curvalue != 0) ? QMF_HIDDEN : 0;
	s_customheight_field[s_current_menu_index].generic.flags	= (s_mode_list[s_current_menu_index].curvalue != 0) ? (QMF_NUMBERSONLY|QMF_HIDDEN) : QMF_NUMBERSONLY;
}
...
static void ApplyChanges( void *unused )
{
	...
	*/
	...
	s_ref_list[!s_current_menu_index].curvalue = s_ref_list[s_current_menu_index].curvalue;
	Q_strncpyz(s_customwidth_field[!s_current_menu_index].buffer, s_customwidth_field[s_current_menu_index].buffer, sizeof(s_customwidth_field[!s_current_menu_index].buffer));
	Q_strncpyz(s_customheight_field[!s_current_menu_index].buffer, s_customheight_field[s_current_menu_index].buffer, sizeof(s_customheight_field[!s_current_menu_index].buffer));
...
//	gamma = (0.8 - (s_brightness_slider[s_current_menu_index].curvalue/10.0 - 0.5)) + 0.5;
	...
	{
		customW  = atoi( s_customwidth_field[s_current_menu_index].buffer );
		customH	= atoi( s_customheight_field[s_current_menu_index].buffer );
		...
		Q_strncpyz( s_customwidth_field[s_current_menu_index].buffer, customStr, sizeof(s_customwidth_field[s_current_menu_index].buffer) );
		s_customwidth_field[s_current_menu_index].cursor = strlen( customStr );
		
		customStr = Cvar_VariableString("r_customheight");
		Q_strncpyz(s_customheight_field[s_current_menu_index].buffer, customStr, sizeof(s_customwidth_field[s_current_menu_index].buffer) );
		s_customheight_field[s_current_menu_index].cursor = strlen( customStr );
	}
...
//			s_ref_list[s_current_menu_index].curvalue = REF_VERITE;
	...
	for ( i = 0; i < 2; i++ )
	{
		...
		s_mode_list[i].generic.callback		= VidModeCallback;	// Knightmare added
		s_mode_list[i].generic.statusbar	= "changes screen resolution";

		// Knightmare- custom mode fields
		s_customwidth_title[i].generic.type	= MTYPE_SEPARATOR;
		s_customwidth_title[i].generic.flags	= (s_mode_list[i].curvalue != 0) ? QMF_HIDDEN : 0;
		s_customwidth_title[i].generic.name	= "custom width";
		s_customwidth_title[i].generic.x		= -16;
		s_customwidth_title[i].generic.y		= 25;

		s_customwidth_field[i].generic.type		= MTYPE_FIELD;
		s_customwidth_field[i].generic.flags		= (s_mode_list[i].curvalue != 0) ? (QMF_NUMBERSONLY|QMF_HIDDEN) : QMF_NUMBERSONLY;
	//	s_customwidth_field[i].generic.name		= "custom width";
		s_customwidth_field[i].generic.callback	= 0;
		s_customwidth_field[i].generic.x			= -104;
		s_customwidth_field[i].generic.y			= 40;
		s_customwidth_field[i].length				= 5;
		s_customwidth_field[i].visible_length		= 6;
		customStr = Cvar_VariableString("r_customwidth");
		Q_strncpyz( s_customwidth_field[i].buffer, customStr, sizeof(s_customwidth_field[i].buffer) );
		s_customwidth_field[i].cursor				= strlen( customStr );

		s_customheight_title[i].generic.type	= MTYPE_SEPARATOR;
		s_customheight_title[i].generic.flags	= (s_mode_list[i].curvalue != 0) ? QMF_HIDDEN : 0;
		s_customheight_title[i].generic.name	= "custom height";
		s_customheight_title[i].generic.x		= 112;
		s_customheight_title[i].generic.y		= 25;

		s_customheight_field[i].generic.type		= MTYPE_FIELD;
		s_customheight_field[i].generic.flags		= (s_mode_list[i].curvalue != 0) ? (QMF_NUMBERSONLY|QMF_HIDDEN) : QMF_NUMBERSONLY;
	//	s_customheight_field[i].generic.name		= "custom height";
		s_customheight_field[i].generic.callback	= 0;
		s_customheight_field[i].generic.x			= 16;
		s_customheight_field[i].generic.y			= 40;
		s_customheight_field[i].length				= 5;
		s_customheight_field[i].visible_length		= 6;
		customStr = Cvar_VariableString("r_customheight");
		Q_strncpyz(s_customheight_field[i].buffer, customStr, sizeof(s_customheight_field[i].buffer) );
		s_customheight_field[i].cursor				= strlen( customStr );

		s_fs_box[i].generic.type		= MTYPE_SPINCONTROL;
		s_fs_box[i].generic.x			= 0;
		s_fs_box[i].generic.y			= 60;
		...
		s_screensize_slider[i].generic.y		= 70;
		...
		s_brightness_slider[i].generic.y			= 80;
	...
	//	s_brightness_slider[i].curvalue				= (1.3 - Cvar_VariableValue("vid_gamma") + 0.5) * 10;
		...
		s_defaults_action[i].generic.y			= 180;
		...
		s_apply_action[i].generic.y			= 190;
	...
	s_npot_mipmap_box.generic.y			= 120;
	...
	// Knightmare- custom mode fields
	Menu_AddItem( &s_software_menu, ( void * ) &s_customwidth_title[SOFTWARE_MENU] );
	Menu_AddItem( &s_software_menu, ( void * ) &s_customwidth_field[SOFTWARE_MENU] );
	Menu_AddItem( &s_software_menu, ( void * ) &s_customheight_title[SOFTWARE_MENU] );
	Menu_AddItem( &s_software_menu, ( void * ) &s_customheight_field[SOFTWARE_MENU] );
	...
	// Knightmare- custom mode fields
	Menu_AddItem( &s_opengl_menu, ( void * ) &s_customwidth_title[OPENGL_MENU] );
	Menu_AddItem( &s_opengl_menu, ( void * ) &s_customwidth_field[OPENGL_MENU] );
	Menu_AddItem( &s_opengl_menu, ( void * ) &s_customheight_title[OPENGL_MENU] );
	Menu_AddItem( &s_opengl_menu, ( void * ) &s_customheight_field[OPENGL_MENU] );
